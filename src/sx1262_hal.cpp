/**
 * sx1262_hal.cpp - Hardware Abstraction Layer for SX1262
 * 
 * VERSION 2 CHANGES:
 * - Removed hardcoded SPI pins (MOSI=11, MISO=13, SCK=12)
 * - Now uses PIN_SX1262_MOSI, PIN_SX1262_MISO, PIN_SX1262_SCK from config.h
 * - Uses SPI_HOST_ID from config.h instead of hardcoded SPI_HOST
 * - Added optional VEXT power control (enabled when PIN_VEXT is defined)
 * - Fully board-agnostic: all pins defined in config.h
 * 
 * USAGE:
 *   For boards with VEXT (e.g., Heltec):
 *     hal_board_power_init();   // Call BEFORE sx1262_driver_init()
 *     sx1262_driver_init();
 *   
 *   For boards without VEXT:
 *     sx1262_driver_init();     // hal_board_power_init() is a no-op
 */
...
hal_result_t hal_spi_init(uint32_t freq_hz, uint8_t cs_pin) {
    ...
    // SPI pins from config.h - board-agnostic configuration
    ...
    // Initialize the SPI Bus using host from config.h
    ...
    err = spi_bus_add_device(SPI_HOST_ID, &devcfg, &spi_device);
    if (err != ESP_OK) {
        return HAL_ERROR_SPI;
    }
...
// For boards with external power control (e.g., Heltec VEXT pin)

#ifdef PIN_VEXT
/**
 * Initialize and enable power to external peripherals
 * Heltec V3: VEXT controls power to SX1262 (active LOW)
 */
hal_result_t hal_board_power_init() {
    hal_result_t res = hal_gpio_init_output(PIN_VEXT);
    if (res != HAL_OK) return res;
    
    // VEXT is active LOW - set LOW to enable power
    hal_gpio_write(PIN_VEXT, 0);
...
void hal_board_power_deinit() {
    // Disable power (VEXT HIGH)
    hal_gpio_write(PIN_VEXT, 1);
    gpio_reset_pin((gpio_num_t)PIN_VEXT);
}
#else
// No VEXT - stub functions
hal_result_t hal_board_power_init() { return HAL_OK; }
void hal_board_power_deinit() { }
#endif
